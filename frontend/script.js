    ...
    async mounted() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (citation) {
                ...
                const msgIdx = parseInt(citation.dataset.msgIdx);
                this.handleCitationClick(index, msgIdx);
            }
        });
    },
    methods: {
        ...
        parseMarkdown(text, msgIdx) {
            const html = marked.parse(text);
            // Convert [1], [2]... to clickable spans with message index reference
            return html.replace(/\[(\d+)\]/g, (match, p1) => {
                return `<span class="citation-link" data-index="${p1}" data-msg-idx="${msgIdx}">${match}</span>`;
            });
        },
        ...
        handleCitationClick(index, msgIdx) {
            const msg = this.messages[msgIdx];
            if (!msg || !msg.ragTrace || !msg.ragTrace.retrieved_chunks) return;
            
            const chunk = msg.ragTrace.retrieved_chunks[index - 1];
            if (chunk) {
                this.activeCitation = {
                    index: index,
                    filename: chunk.filename || '未知来源',
                    page_number: chunk.page_number || 0,
                    text: chunk.text || '无正文内容'
                };
            }
        },

        handleDragOver(e) {
            if (this.isAdmin && this.activeNav === 'settings') {
                e.preventDefault();
        ...
        handleDrop(e) {
            if (!this.isAdmin || this.activeNav !== 'settings') return;
            e.preventDefault();
            this.isDragOver = false;
